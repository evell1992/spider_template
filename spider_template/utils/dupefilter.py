...
class MFPDupeFilter(BaseDupeFilter):
    ...
    def request_seen(self, request):
        ...
        if template.stage != len(template.pipeline) - 1:
            return False
        ...
        data = self.mongo.find_one('dupefilter', {"_id": record['_id']})
        if not data:
            try:
                self.mongo.update('dupefilter', {"_id": record['_id']}, record, upsert=True)
